...
export default function ContactsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await getItems<Contact>('contacts');
        if (!cancelled) setContacts(data);
      } catch (err) {
        if (!cancelled) setError(err instanceof StorageError ? err.message : 'Failed to load contacts');
      } finally {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      if (editingId) {
        const updated = await updateItem<Contact>('contacts', editingId, {
          ...form,
          updatedAt: now,
        });
        setContacts(updated);
        ...
        const updated = await addItem<Contact>('contacts', newContact);
        setContacts(updated);
      }
      closeModal();
    } catch (err) {
      setError(err instanceof StorageError ? err.message : 'Failed to save contact');
    }
  }, [form, editingId, closeModal]);

  const handleDelete = useCallback(async (id: string) => {
    try {
      const updated = await removeItem<Contact>('contacts', id);
      setContacts(updated);
    } catch (err) {
      setError(err instanceof StorageError ? err.message : 'Failed to delete contact');
    }
  }, []);
