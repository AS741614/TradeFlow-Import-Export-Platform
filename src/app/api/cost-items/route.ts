...
export async function GET(req?: NextRequest) {
  try {
    ...
    const config = {
      searchColumns: [costItems.category, costItems.description],
      sortByWhitelist: ['category', 'amount', 'id'],
      sortByColumnMap: {
        category: costItems.category,
        amount: costItems.amount,
        id: costItems.id,
      },
      defaultSortColumn: costItems.id,
    };

    const { whereClause, orderClause } = applySearchFilterSort(searchParams, config);

    const db = getDb();
    let conditions = withTenant(costItems, session.orgId);
    if (whereClause) {
    ...
    const list = await getCostItems(session.orgId, limit, offset, whereClause, orderClause);
