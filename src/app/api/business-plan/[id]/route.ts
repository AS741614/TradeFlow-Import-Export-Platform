...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const record = await getBusinessPlanSectionById(session.orgId, id);
    if (!record) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const before = await getBusinessPlanSectionById(session.orgId, id);
    if (!before) {
      return NextResponse.json({ error: 'Business plan section not found' }, { status: 404 });
    }

    const record = await updateBusinessPlanSection(session.orgId, id, parsed.data);
    if (!record) {
      return NextResponse.json({ error: 'Business plan section not found' }, { status: 404 });
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const record = await deleteBusinessPlanSection(session.orgId, id, session.userId, reason);
    if (!record) {
      return NextResponse.json({ error: 'Business plan section not found' }, { status: 404 });
    }
    return NextResponse.json({ data: record });
