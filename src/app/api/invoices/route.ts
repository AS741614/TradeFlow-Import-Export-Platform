...
export async function GET(req?: NextRequest) {
  try {
    ...
    const config = {
      searchColumns: [invoices.number],
      statusColumn: invoices.status,
      sortByWhitelist: ['number', 'currency', 'status', 'total', 'issuedDate', 'dueDate', 'createdAt'],
      sortByColumnMap: {
      ...
      defaultSortColumn: invoices.createdAt,
    };

    const { whereClause, orderClause } = applySearchFilterSort(searchParams, config);

    const db = getDb();
    let conditions = withTenant(invoices, session.orgId);
    if (whereClause) {
    ...
    const list = await getInvoices(session.orgId, limit, offset, whereClause, orderClause);
