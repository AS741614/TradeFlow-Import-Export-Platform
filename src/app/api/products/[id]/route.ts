...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const record = await getProductById(session.orgId, id);
    if (!record) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const before = await getProductById(session.orgId, id);
    if (!before) {
      return NextResponse.json({ error: 'Product not found' }, { status: 404 });
    }

    const record = await updateProduct(session.orgId, id, parsed.data);
    if (!record) {
      return NextResponse.json({ error: 'Product not found' }, { status: 404 });
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const record = await deleteProduct(session.orgId, id, session.userId, reason);
    if (!record) {
      return NextResponse.json({ error: 'Product not found' }, { status: 404 });
    }
    return NextResponse.json({ data: record });
