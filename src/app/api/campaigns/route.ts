...
export async function GET(req?: NextRequest) {
  try {
    ...
    const config = {
      searchColumns: [campaigns.name, campaigns.subjectLineA],
      statusColumn: campaigns.status,
      sortByWhitelist: ['name', 'subjectLineA', 'status', 'createdAt'],
      sortByColumnMap: {
        name: campaigns.name,
        subjectLineA: campaigns.subjectLineA,
        status: campaigns.status,
        createdAt: campaigns.createdAt,
      },
      defaultSortColumn: campaigns.createdAt,
    };

    const { whereClause, orderClause } = applySearchFilterSort(searchParams, config);

    const db = getDb();
    let conditions = withTenant(campaigns, session.orgId);
    if (whereClause) {
    ...
    const list = await getCampaigns(session.orgId, limit, offset, whereClause, orderClause);
